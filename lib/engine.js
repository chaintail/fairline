...
export function buildEnvelope({ mode, nowMs, ft, state, model, consensus }) {
  // Settled totals lines (line < goals already scored) no longer trade: the
  // market pulls them, so its last tick is stale — publishing an "edge"
  // against it would be nonsense. Model side keeps the line (priced at
  // certainty); consensus and edge are dropped.
  const totalGoals = state.goalsHome + state.goalsAway;
  const liveTotals = Object.fromEntries(
    Object.entries(consensus.totals || {}).filter(([line]) => Number(line) > totalGoals)
  );
  consensus = { ...consensus, totals: liveTotals };
  return {
    v: 1,
    producer: "fairline",
    mode,
