...
// Cup Final feed (fixture 18257739): 1 = not started, 2 = first half,
// 3 = half-time break, 4 = second half (each confirmed directly at its
// real kickoff/break event), 6 = awaiting extra time (the break after full
// time, mirroring 3's role — Clock stopped at exactly 5400 = 90:00),
// 7 = extra time first half, 9 = extra time second half (both confirmed
// directly at their real kickoff events, Clock resuming at 5400 and 6300).
// The match finished 1-0 after extra time (a real, standing goal in ET1,
// confirmed via the feed's own `game_finalised`/StatusId+Period=100
// finalisation record — its own documented definition of an authoritative
// final state) — no penalties were needed. StatusId 10 was initially
// (wrongly) mapped to "PE" here on the assumption it meant a shootout;
// real data showed otherwise (it's the ~5-6 minute pre-finalisation
// transition window between ET2 ending and `game_finalised` landing), so
// that mapping has been removed rather than shipped mislabeled — it falls
// through to the generic "past H2 → END" fallback below like every other
// unconfirmed code. Code 5, 8, and 100 (game_finalised itself, and
// anything else) are not individually mapped for phase — any numeric code
// past 4 reads as "END": imprecise but never actively misleading, unlike
// the original GameState-string fallback (which reads as pre-match).
function phaseFromStatusId(statusId) {
  switch (statusId) {
    ...
    case 7: return "ET1";
    case 9: return "ET2";
    default: return typeof statusId === "number" && statusId > 4 ? "END" : null;
  }
}

/**
 * The scores endpoint is an append-only event log; not every event carries
 * match state. Purely administrative/commentary events (Action "comment",
 * "lineups", "connected", etc — confirmed in the real fixture 18257739 feed)
 * have neither StatusId nor Clock at all. Picking literally the
 * highest-Seq event as "current state" is wrong when that event happens to
 * be one of these — use this to find the latest event that actually
 * carries state instead.
 */
export function hasScoreState(e) {
  if (!e) return false;
  const statusId = e.StatusId ?? e.statusId;
  return (statusId !== undefined && statusId !== null) || (e.Clock ?? e.clock) != null;
}

/**
 * Not every event that carries phase/clock also carries Score — the total
 * goals/cards/corners tally is only attached to specific notable events
 * (goals, cards, corners, finalisation), not every possession/clock tick.
 * Picking one "latest" event for everything (even after fixing which event
 * that is) can pair a fresh clock with an empty Score — the real cause of a
 * live P1 where the site showed 0 goals well after a real, standing goal
 * (confirmed via the fixture's own `game_finalised`/StatusId+Period=100
 * event, the feed's own definition of an authoritative final record).
 * Build one composite from the full history instead: phase/clock from the
 * latest state-carrying event, Score independently from the latest event
 * that actually carries Score data (preferring a finalisation event if one
 * exists, since that's authoritative over any earlier goal/VAR sequence).
 */
export function mergeScoreEvents(events) {
  const list = (events || []).filter(Boolean);
  const byMaxSeq = (a, b) => ((a.Seq ?? a.seq ?? -1) > (b.Seq ?? b.seq ?? -1) ? a : b);
  const stateCandidates = list.filter(hasScoreState);
  const latestState = stateCandidates.length ? stateCandidates.reduce(byMaxSeq) : null;
  const scoreCandidates = list.filter((e) => (e.Score ?? e.score) != null);
  const finalised = list.filter((e) => {
  ...
  const scoreSource = finalised.length ? finalised.reduce(byMaxSeq) : (scoreCandidates.length ? scoreCandidates.reduce(byMaxSeq) : null);
  if (!latestState && !scoreSource) return null;
  return {
    ...(latestState || {}),
    Score: scoreSource ? (scoreSource.Score ?? scoreSource.score) : (latestState && (latestState.Score ?? latestState.score)),
  };
}
...
export function scoreToState(payload) {
  ...
  const phase = phaseFromStatusId(statusId) ?? normalizePhase(g(payload, "GameState", "gameState"));
