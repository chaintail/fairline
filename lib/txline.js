...
// Cup Final feed (fixture 18257739) across the actual match (finished 0-0,
// so it went to extra time — a rare real end-to-end look at this part of
// the feed): 1 = not started, 2 = first half, 3 = half-time break,
// 4 = second half (each confirmed directly at its real kickoff/break
// event), 6 = awaiting extra time (the break after full time, mirroring
// 3's role — Clock stopped at exactly 5400 = 90:00), 7 = extra time first
// half, 9 = extra time second half (both confirmed directly at their real
// kickoff events: Action "kickoff"/"kickoff_team", Clock resuming at 5400
// and 6300 respectively — identical pattern to the H1/H2 kickoffs).
// 10 = penalties (confirmed directly: appeared right as ET2 ended level,
// with Clock reset to Seconds:0/Running:false — a shootout genuinely has
// no match clock, so this isn't the "clock stuck at 0" bug pattern, it's
// the correct real state). Code 5 and 8 (an HTET break presumably sits at
// 8, mirroring 3/6's pattern, but wasn't directly observed — may have
// rolled out of a windowed poll before the fix) and anything past 10
// (final full time) are not individually confirmed — any other numeric
// code is treated as "END": imprecise, but far less wrong than falling
// back to the GameState string (which reads as pre-match). Matches the
// existing "regulation only" limitation documented above for whatever
// remains unconfirmed.
