...
import { normalizedTime, oddsToConsensus, scoreToState, hasScoreState, isTerminal } from "./txline.js";
...
export async function liveTick() {
  ...
  // snapshot is an event list; the latest event that actually carries state
  // (StatusId/Clock) reflects current match state — some events (comments,
  // lineups, etc) carry neither and must not be picked as "current" just
  // because they have the highest Seq.
  const stateEvents = events.filter(hasScoreState);
  const latest = (stateEvents.length ? stateEvents : events)
    .reduce((a, b) => ((a?.Seq ?? -1) > (b?.Seq ?? -1) ? a : b), null);
...
export async function liveEdgeSeries(stepMs = 60_000, maxPoints = 400, sinceMs = 0) {
  ...
  // Exclude state-less events (comments, lineups, etc) before reducing —
  // otherwise a bare annotation at some timestamp would wrongly compute as
  // an NS/zeroed state and corrupt whichever sampled row lands on it.
  const events = (Array.isArray(scores) ? scores : [])
    .filter(hasScoreState)
